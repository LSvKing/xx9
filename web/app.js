...
// 取播放数据：后端实时拉签名地址偶发抽风(代理/WAF) → 拿到空线路时隔 1.5s 自动重试一次
async function fetchVideo(id, retried) {
  const res = await api('/api/video/' + id);
  if (!res.ok) return { gone: true };
  const v = await res.json();
  const hasLines = (v.lines && v.lines.length) || (v.sources && v.sources.length);
  if (!hasLines && !retried) {
    await new Promise(r => setTimeout(r, 1500));
    if (curId !== id) return { aborted: true };   // 期间切走了就别再放
    return fetchVideo(id, true);
  }
  return { v };
}
